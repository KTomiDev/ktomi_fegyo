...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsPedArmed(player, 7) then
            ...
            local bool,tarbanlevo = GetAmmoInClip(player,weapon)

            status['armed'] = true

            for key,value in pairs(fegyok) do
  
                for keyTwo,valueTwo in pairs(fegyok[key]) do
                    if weapon == GetHashKey('weapon_'..keyTwo) then
                        status['weapon'] = keyTwo

                        szovegmutatas(0.824, 1.455, 0.9,1.0,0.50, "~b~Ammo:~w~  ".. tarbanlevo .. '', 255, 255, 255, 255)

                    end
                end
            end
        end
        local fegyo = GetCurrentPedWeapon(NetworkGetEntityOwner(GetPlayerPed(-1)))
        szovegmutatas(0.954, 1.445, 1.0,1.0,0.40, "~w~  ".. jatekosadat.job.label ..  ' - ' .. jatekosadat.job.grade_label, 255, 255, 255, 255)
        Citizen.Wait(1)
    end
end)
--GetAmmoInPedWeapon
function szovegmutatas(x,y ,width,height,scale, text, r,g,b,a, outline)
    SetTextFont(4) -- 7
